...
import { CheckoutRequestDTO } from '../dto/CheckoutRequestDTO';
 ...
 * 5. DISCOUNT VALIDATION: Subtotal (pre-discount) vs finalTotal (post-discount) wajib divalidasi terpisah (BUG-01 FIX).
 */
export class CheckoutUseCase {
  ...
  async execute(request: CheckoutRequestDTO): Promise<string> {
    ...
    return withLock(lockKey, async () => {
      ...
      const calculatedSubtotal = request.items.reduce((sum, item) => MathUtils.add(sum, MathUtils.mul(item.price, item.quantity)), 0);
      const totalDiscount = MathUtils.add((request.manualDiscountAmount || 0), (request.loyaltyDiscountAmount || 0));
      const discountPercentage = calculatedSubtotal > 0 ? MathUtils.div(totalDiscount, calculatedSubtotal) : 0;

      // Rule 1: High Discount (> 30%) requires authorization
      if (discountPercentage > 0.3 && !request.authorizedBy && request.userRole !== 'ADMIN') {
        throw new Error(`Diskon terlalu besar (${Math.round(discountPercentage * 100)}%) // UI-only: format string. Otorisasi Manager diperlukan.`);
      }
      ...
      while (true) {
        ...
        try {
          return await this.unitOfWork.execute(async () => {
           ...
           // FIX BUG-01: Calculate pre-discount subtotal separately for validation
           ...
           // FIX BUG-01: Validate SUBTOTAL (pre-discount) first
           calculatedTotal = MathUtils.roundInt(calculatedTotal);
           if (MathUtils.roundInt(request.subtotal || 0) !== calculatedTotal) {
               throw new Error(`Manipulasi Subtotal Terdeteksi: Subtotal Klien: ${request.subtotal}, Subtotal Terhitung: ${calculatedTotal}`);
           }

           // FIX BUG-01: Validate FINAL TOTAL (post-discount) consistency
           const expectedFinalTotal = MathUtils.roundInt(MathUtils.sub(calculatedTotal, totalDiscount));
           if (MathUtils.roundInt(request.total) !== expectedFinalTotal) {
               throw new Error(`Manipulasi Total Final Terdeteksi: Total Klien: ${request.total}, Total Ekspektasi: ${expectedFinalTotal}`);
           }
           ...
           if (openShift) {
             ...
             // FIX BUG-02 PREP: Use cashPortion field if SPLIT, else full amount if CASH
             const addedCash = paymentMethod === 'CASH' ? finalTotal : (paymentMethod === 'SPLIT' ? (request.cashPortion || 0) : 0);
