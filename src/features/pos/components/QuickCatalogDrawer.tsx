/**
 * @ai_context: Drawer katalog cepat — READ ONLY, cek harga & stok
 * @business_rule: Query via DIContainer.stockRepository, BUKAN db langsung (RULE-01 compliance)
 * @security_tier: LOW — display only, no mutations
 */
...
export function QuickCatalogDrawer({ isOpen, onClose }: QuickCatalogDrawerProps) {
  ...
  const [products, setProducts] = useState<StockItemWithId[]>([]);

  const loadProducts = useCallback(async () => {
    if (!isOpen) return;
    const all = await DIContainer.stockRepository.list();
    const term = searchTerm.toLowerCase();
    const filtered = all
      .filter((item) => {
        const matchSearch = !term ||
          item.name.toLowerCase().startsWith(term) ||
          (item.barcode ?? '').toLowerCase().startsWith(term);
        const matchTab = tab === 'perhiasan'
          ? item.category === StockCategory.GOLD_JEWELLERY || item.category === StockCategory.IMITATION
          : item.category === StockCategory.GOLD_BAR || item.category === StockCategory.ACCESSORIES;
        return matchSearch && matchTab;
      })
      .slice(0, 30) as StockItemWithId[];
    setProducts(filtered);
  }, [isOpen, searchTerm, tab]);
  ...
  return (
    <div
      className="fixed inset-y-0 left-0 w-full md:w-96 bg-white shadow-2xl z-50 flex flex-col animate-in slide-in-from-left duration-300 border-r border-stone-200"
      data-component-id="quick-catalog-drawer"
      data-error-domain="pos.catalog"
    >
      ...
      <div className="p-4 border-b border-stone-200">
        ...
        <div className="flex p-1 bg-stone-100 rounded-xl">
          <button onClick={() => setTab('perhiasan')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors \${tab === 'perhiasan' ? 'bg-white text-brand-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>Perhiasan</button>
          <button onClick={() => setTab('bahan')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors \${tab === 'bahan' ? 'bg-white text-brand-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>Bahan &amp; Brand</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-stone-50">
        {products.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-stone-500 text-sm">Produk tidak ditemukan</p>
          </div>
        ) : (
          products.map((item) => (
