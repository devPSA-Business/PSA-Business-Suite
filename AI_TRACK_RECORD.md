...
| 2026-05-15 | **KOMPREHENSIF AI REVIEW & PERBAIKAN BATCH**: (1) Fix `VoidTransactionUseCase.ts` — hapus `return voidedTransaction` yang salah type di void method, tambah `payloadDiff` di audit log. (2) Fix `dataArchiver.ts` — hapus duplikat JSDoc block (copy-paste artifact). (3) Fix `DatabaseAdminServiceImpl.ts` — tambah type safety penuh (hapus semua `any`), concurrent `Promise.all`, `recoverFromCloud` dengan proper typing, logging. (4) Update `IDatabaseAdminService.ts` — tambah `recoverFromCloud()` ke interface (sebelumnya tidak ada). (5) Fix `vite.config.ts` — tambah `VITE_BASE_PATH` env var + `base: basePath` untuk GitHub Pages support; fix start_url dan scope PWA manifest. (6) Buat `.github/workflows/pages.yml` — GitHub Pages deployment workflow (build dengan `VITE_BASE_PATH=/PSA-Business-Suite/` + 404.html SPA fallback + .nojekyll). (7) Fix `HomePage.tsx` — link `/dashboard` (deprecated) → `/executive`. (8) Buat `docs/TD.md` — Technical Debt Registry resmi dengan 15 entri (TD-01 sampai TD-15). | Selesai | Menengah |
